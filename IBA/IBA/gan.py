...
class Generator(torch.nn.Module):
    ...
    def __init__(self, model, layer, image, dev='cpu', capacity=None, batch_size=None):
        super().__init__()
        self.image = image
        # use image size
        if capacity is not None:
          image_mask_param = torch.tensor(_to_saliency_map(capacity.cpu().detach().numpy(), image.shape[1:], data_format="channels_first")).to(dev)
          self.image_mask_param = image_mask_param.expand(image.shape[0], -1, -1).clone().unsqueeze(0)
        else:
          self.image_mask_param = torch.zeros(image.shape, dtype=torch.float).to(dev)
...
class Discriminator(torch.nn.Module):
    def __init__(self, channels, size):
        ...
        self.main_module = nn.Sequential(
            ...
            # State (256x16x16)
            nn.Conv2d(in_channels=channels*2, out_channels=channels*4, kernel_size=4, stride=2, padding=1),
...
class WGAN_CP(object):
    def __init__(self, model, layer,
                 ...
                 dataset_size=800, subdataset_size=50, lr=0.00005, batch_size=32, weight_cliping_limit=0.01,
                 generator_iters=200, critic_iter=5, dev='cpu'):
        ...
        self.G = Generator(model, layer, image, dev=self.dev, capacity=feature_mask, batch_size=batch_size)
        ...
        for idx_subset in range(self.num_subdataset):
          sub_dataset = torch.unsqueeze(self.feature_map, 0).expand(self.subdataset_size, -1, -1, -1)
          noise = torch.zeros_like(sub_dataset).normal_()
          std = random.uniform(0, 5)
          mean = random.uniform(-2, 2)
          noise = std * noise + mean
          sub_dataset = self.feature_mask * sub_dataset + (1 - self.feature_mask) * noise
          if dataset is None:
            dataset = sub_dataset
          else:
    ...
    def train(self, dev):
        ...
        optimizer_G = torch.optim.RMSprop([
          {"params":generator.mean, "lr":0.1}, 
          {"params":generator.eps, "lr":0.05}, 
          {"params":generator.image_mask_param, "lr":0.003}]
          )
        ...
        for epoch in range(self.generator_iters):
            ...
            for i, imgs in enumerate(self.dataloader):
                ...
                z = z.unsqueeze(0).expand(imgs.shape[0], -1, -1, -1).clone().normal_().to(dev)
                # std = random.uniform(0, 5)
                # mean = random.uniform(-2, 2)
                # noise = std * noise + mean
                ...
                if i % self.critic_iter == 0:
                    ...
                    optimizer_G.step()
                    if epoch % 10 == 0:
                      self.image_mask_history.append(generator.image_mask().clone().detach().cpu().numpy())
                    print(
