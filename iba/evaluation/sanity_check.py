...
class SanityCheck(BaseEvaluation):
    ...
    def evaluate(  # noqa
        ...
        Args:
            ...
            check (str, optional): which component to check, can be either 'gan' or 'input_iba'.
        ...
        for l in perturb_layers:
            ...
            ssim_val = self.sanity_check_single(input_tensor=input_tensor,
                                                target=target,
                                                attr_cfg=attr_cfg,
        ...
        if save_heatmaps:
            self.attributor.show_mask(heatmap, out_file=osp.join(save_dir, 'ori_input_mask'))
    ...
    def sanity_check_single(self,
        ...
        _ = self.attributor.train_feat_iba(input_tensor, closure, attr_cfg['iba'])
        if check == 'gan':
            perturb_model(self.attributor.classifier, perturb_layers)
            gen_input_mask = self.attributor.train_gan(input_tensor, attr_cfg['gan'])
        else:
            gen_input_mask = self.attributor.train_gan(input_tensor, attr_cfg['gan'])
            perturb_model(self.attributor.classifier, perturb_layers)
        input_mask, _ = self.attributor.train_input_iba(
            self.attributor.cfg['input_iba'],
            input_tensor,
        ...
        if save_heatmaps:
            ...
            self.attributor.show_mask(
                input_mask,
                out_file=osp.join(save_dir, f"{perturb_layers[-1]}_{ssim_val:.3f}"))
        return ssim_val
