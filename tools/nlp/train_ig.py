...
cfg = mmcv.Config.fromfile(
    os.path.join(os.getcwd(), '../configs/deep_lstm.py'))
...
def collate_batch(batch):
    ...
    for (_label, _text, _fname) in batch:
        ...
        fname_list.append(
            int(_fname.split('/')[-1].split('.')[0].replace('_', '')))
    ...
    return label_list.to(device), padded_text_list.to(
        device), text_length_list.to(device), fname_list.to(device)
...
def generate_ig_masks():
    ...
    dataloader = DataLoader(
        train_iter, batch_size=1, shuffle=False, collate_fn=collate_batch)
    model = input_iba.models.model_zoo.build_classifiers()
    for count, (label, text, text_length,
                fname) in tqdm(enumerate((iter(dataloader)))):
        if count >= 2000:
            break
        filename = filename.split('/')[-1].split('.')[0]
        mask_file = os.path.join(work_dir, filename)
        if not os.path.isfile(mask_file + '.png'):
            attributor = LayerIntegratedGradients(
                model_fn_given_length(text_length[0].unsqueeze(0), model),
                model.embedding)
